...
| 32 | P1 | Action payloads not validated against typed schemas; alert email has no recipient allow-list | AI-safety audit, `email_actions.py` | 🟡 partial — auto-alert email now validates recipient format + optional `ALERT_EMAIL_ALLOWLIST` domain gate (`test_action_safety.py`). Typed per-action payload schemas still tracked. |
