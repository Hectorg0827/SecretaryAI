...
| 31 | P1 | PolicyEngine documented as enforced pre-COMMIT but NOT called on the approval-execute path | AI-safety audit, `actions.py`/`domain/policy.py` | ⬜ open — wire `PolicyEngine.evaluate` into `_execute_approved_draft` (amount thresholds). |
| 32 | P1 | Action payloads not validated against typed schemas; alert email has no recipient allow-list | AI-safety audit, `actions/engine.py`/`email_actions.py` | ⬜ open |
| 33 | P1 | Prompt injection — untrusted email/ingested content interpolated into prompts without hard delimiting | AI-safety audit, `dashboard.py`/`chat.py` | ⬜ open (blast radius limited: chat can't execute; still fix) |
| 34 | P2 | Portal credentials persisted in `browser_jobs.parameters` in cleartext | AI-safety audit, `access_router.py` | ⬜ open |
| 35 | P2 | Computer-Use screenshot redaction is best-effort (no-op if pytesseract missing); strong PII check not called in loop | AI-safety audit, `screenshot_manager.py`/`safety.py` | ⬜ open |
