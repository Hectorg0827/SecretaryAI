...
| 18 | P2 | No `test` script in frontend/desktop package.json | package.json | 🟡 mostly — frontend now has vitest + tests (`npm run test`, wired in CI, covers the 401→refresh security flow); backend pytest + Rust `cargo test` in CI. Desktop JS still relies on typecheck/lint/build (no unit runner yet). |
